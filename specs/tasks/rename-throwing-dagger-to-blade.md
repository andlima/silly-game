...
A prior run of this spec (`task-20260425T2303579a1785-20260425T230357781196`,
PR #69) actually applied the rename correctly and was approved by the
independent reviewer, but the orchestrator's review-decision-gate failed to
parse the reviewer's JSON output (the file began with stray prose,
`{1F52A}\` HUD glyph...`, instead of a JSON object). The implementation work
was sound; only the gate broke. This re-run starts fresh on a new branch.
The implementing agent should treat PR #69 as abandoned — do not push to
its branch; let the orchestrator open a new PR.

To minimise the chance of a repeat, this spec avoids `\u{1F52A}` escape
notation in its prose and instead refers to the kitchen-knife glyph by its
Unicode code point name (U+1F52A). Source-file Unicode escapes are still
fine; only this document avoids them.
...
The HUD glyph (the kitchen-knife emoji at code point U+1F52A) stays
unchanged; this is purely a name change.
...
kitchen-knife emoji (U+1F52A). The `t` key still triggers the throw, and
the abstract action types (`throw`, `throwDir`, `throwCancel`,
`throwPending`) **stay as-is** — they describe the action, not the weapon.
...
- The melee `dagger` equipment (item type `'dagger'`, melee glyph) — out of
...
- `GLYPHS_ENHANCED.throwing_dagger` → `throwing_blade` (key only — the
  `char` value stays exactly as it is per the prior `swap-dagger-emojis`
  spec; the trailing comment may stay or be updated to "throwing blade
  glyph" — either is fine).
...
- `specs/throwing-dagger.md` — rewrite the description, prose, acceptance
  ...
  — switch to "the blade flies", "the blade hits". The bottom design-note
  paragraph that compares the throwing weapon's emoji against the melee
  dagger's emoji (kitchen-knife vs sword-like dagger) stays accurate;
  rephrase as needed for legibility (e.g. "the kitchen-knife glyph for the
  throwing blade vs the dagger glyph for the melee dagger").
- `specs/merchant-npc.md` — update the references at lines 16, 41, 48–49,
  58, 108–109, and 234. Concretely: `food, throwing daggers, equipment` →
  `food, throwing blades, equipment`; `'throwing_dagger' | 'equipment'` →
  `'throwing_blade' | 'equipment'`; `throwing_dagger OR a random
  equipment piece` → `throwing_blade OR ...`; the table row
  `| throwing_dagger | 2 |` → `| throwing_blade | 2 |`; the
  `inventory.throwingDaggers += 1` line and surrounding sentence → use the
  blade names; the "Dependency wiring on the 'throwing-dagger' spec: if
  throwing daggers are…" sentence at line 234 → "…if throwing blades are…"
  (the spec-id reference `'throwing-dagger'` stays since the spec id is
  unchanged).
...
3. Manual smoke test in the browser (`npm run dev` or open `index.html`):
   - Pick up a throwing blade — message reads "You pick up a throwing
     blade (N total)."
   - HUD shows the kitchen-knife glyph + count under the new id `hud-blades`.
...
- The action types and `throwPending` flag are intentionally untouched.
  ...
  is renamed.
- The HUD icon stays as the kitchen-knife glyph (code point U+1F52A). After
  the change, verify by reading `index.html` lines 853 and 855 — the
  existing emoji literal there should be byte-identical to what was in the
  file before.
...
- The spec frontmatter's `description:` field on
  ...
  that spec is ever re-implemented. Keep that description ASCII-only (no
...
- Commit messages and PR titles produced by the implementing agent must
  ...
  descriptions, and the orchestrator publishes the spec description there.
  This spec's frontmatter `description` is already emoji-free; keep it
  that way.
